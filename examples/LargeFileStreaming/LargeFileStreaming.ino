...
void setup() {
    ...
    DbFileUploadPullCb pullCb = [&producer](size_t requested, uint8_t *buffer, size_t &produced, bool &eof) -> DbStatus {
        ...
        if ((producer.offset % (64 * 1024)) == 0 || eof) {
            Serial.printf("write progress: %u / %u bytes\n",
    ...
    if (!sizeRes.status.ok() || sizeRes.value != kPayloadSize) {
        Serial.printf("fileSize mismatch: status=%s size=%u\n",
    ...
    if (readRes.value != kPayloadSize || sink.total != kPayloadSize || sink.hash != producer.hash) {
        Serial.printf("verification failed (read=%u sink=%u expected=%u)\n",
    ...
    Serial.printf("Large chunked stream verified: %u bytes, hash=0x%08lX\n",
                  static_cast<unsigned>(sink.total),
