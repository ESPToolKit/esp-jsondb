...
void setup() {
	...
	auto stFail = db.updateById("users", r2.value, [](DocView &v) { v["email"].set("a@x.com"); });
	Serial.printf(
	    "Update to duplicate email: %s\n",
	    stFail.ok() ? "OK (unexpected)" : stFail.message
	);

	// Update u2's email to a new unique value (should succeed)
	auto stOk = db.updateById("users", r2.value, [](DocView &v) { v["email"].set("c@x.com"); });
