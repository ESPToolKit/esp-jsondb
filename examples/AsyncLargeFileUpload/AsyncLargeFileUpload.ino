...
void setup() {
    ...
    auto start = db.writeFileStreamAsync("uploads/large_payload.bin", largeUploadPull, opts, onLargeUploadDone);
    if (!start.status.ok()) {
    ...
    Serial.printf("Started async upload %lu for %u bytes\n",
...
void loop() {
    ...
    if (!gUploadDone) {
        ...
        if (state.status.ok()) {
            Serial.printf("state=%u progress=%u/%u\n",
    ...
    if (!verified) {
        ...
        if (!sizeRes.status.ok() || sizeRes.value != kPayloadSize) {
            Serial.printf("fileSize mismatch: %s size=%u\n",
        ...
        if (readRes.value != kPayloadSize || sink.total != kPayloadSize || sink.hash != gExpectedHash) {
            Serial.printf("verify failed read=%u sink=%u expected=%u hash=0x%08lX/0x%08lX\n",
        ...
        Serial.printf("Async large file verified: %u bytes, hash=0x%08lX\n",
                      static_cast<unsigned>(sink.total),
                      static_cast<unsigned long>(sink.hash));
    }
