...
void setup() {
	...
	auto st1 = db.updateOne(
	    "users",
	    [&](const DocView &v) {
		    return v["email"].as<std::string>() ==
		           std::string("user@example.com"); // match by email
	    },
	    [&](DocView &v) {
	...
	// Verify results
	auto foundExisting = db.findOne("users", [&](const DocView &v) {
		return v["email"].as<std::string>() == std::string("user@example.com");
	});
