...
void ESPJsonDB::rebindAllocatorAwareStateLocked(bool preserveData) {
	...
	CollectionConfigMap newCollectionConfigs{
	    std::less<std::string>{}, CollectionConfigMap::allocator_type(usePSRAMBuffers)};
...
DbStatus ESPJsonDB::preloadCollectionFromFsByName(
	...
	auto col = std::make_unique<Collection>(
	    *this, name, sc, _baseDir, collectionCfg, _cfg.usePSRAMBuffers, *_fs
	);
...
static void listDirEntries(
    fs::FS &fsImpl, const std::string &dir, DirEntryVector &out
) {
	FrLock fs(g_fsMutex);
	...
	for (auto kv : cols) {
		...
		for (JsonObjectConst obj : arr) {
			...
			if (!DocId::isHex(id, std::strlen(id))) {
				return setLastError({DbStatusCode::InvalidArgument, "snapshot contains invalid _id"});
			}
